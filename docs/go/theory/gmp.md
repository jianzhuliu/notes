...
!> 本文来源 [https://learnku.com/articles/41728](https://learnku.com/articles/41728 ':crossorgin :target=_blank') 
...
![logo](/_media/gmp/2.png)

在多进程 / 多线程的操作系统中，就解决了阻塞的问题，因为一个进程阻塞 cpu 可以立刻切换到其他进程中去执行，而且调度 cpu 的算法可以保证在运行的进程都可以被分配到 cpu 的运行时间片。这样从宏观来看，似乎多个进程是在同时被运行。但新的问题就又出现了，进程拥有太多的资源，进程的创建、切换、销毁，都会占用很长的时间，CPU 虽然利用起来了，但如果进程过多，CPU 有很大的一部分都被用来进行进程调度了。

> 怎么才能提高 CPU 的利用率呢？

但是对于 Linux 操作系统来讲，cpu 对进程的态度和线程的态度是一样的。

![logo](/_media/gmp/3.png)
...
![logo](/_media/gmp/4.png)

这样，我们再去细化去分类一下，内核线程依然叫 “线程 (thread)”，用户线程叫 “协程 (co-routine)”.

![logo](/_media/gmp/5.png)
...
![logo](/_media/gmp/9.png)

> 下面我们来看看被废弃的 golang 调度器是如何实现的？

![logo](/_media/gmp/10.png)
...
面对之前调度器的问题，Go 设计了新的调度器。在新调度器中，出列 M (thread) 和 G (goroutine)，又引进了 P (Processor)。

![logo](/_media/gmp/11.png)
...
[trace](/_media/gmp/trace.go ':include :type=code')
...
2020/10/23 18:06:30 Parsing trace...
2020/10/23 18:06:30 Splitting trace...
2020/10/23 18:06:30 Opening browser. Trace viewer is listening on http://127.0.0.1:55785
``` 

我们可以通过浏览器打开 http://127.0.0.1:55785 网址，点击 view trace 能够看见可视化的调度流程。

![logo](/_media/gmp/15.png)

![logo](/_media/gmp/16.png)

?> G 信息

点击 Goroutines 那一行可视化的数据条，我们会看到一些详细的信息

![logo](/_media/gmp/17.png)
...
![logo](/_media/gmp/18.png)

> 一共有两个 M 在程序中，一个是特殊的 M0，用于初始化使用，这个我们不必讨论。

?> P 信息

![logo](/_media/gmp/19.png)
...
![logo](/_media/gmp/22.png)

## (2) 场景 2

G1 运行完成后 (函数：goexit)，M 上运行的 goroutine 切换为 G0，G0 负责调度时协程的切换（函数：schedule）。从 P 的本地队列取 G2，从 G0 切换到 G2，并开始运行 G2 (函数：execute)。实现了线程 M1 的复用。

![logo](/_media/gmp/23.png)

## (3) 场景 3

假设每个 P 的本地队列只能存 3 个 G。G2 要创建了 6 个 G，前 3 个 G（G3, G4, G5）已经加入 p1 的本地队列，p1 本地队列满了。

![logo](/_media/gmp/24.png)
...
![logo](/_media/gmp/31.png)

## (11) 场景 11

G8 创建了 G9，假如 G8 进行了非阻塞系统调用。

![logo](/_media/gmp/32.png)
